...
class ATL_NO_VTABLE IDispatchExImpl : public BASE_ITF
{
	...
	typedef CMap<CString,LPCTSTR,CDynamicDispatchEntry*,CDynamicDispatchEntry*&> CMapNameToEntry;
	typedef CMap<DISPID,DISPID,CDynamicDispatchEntry*,CDynamicDispatchEntry*&> CMapDispIDToEntry;
	...
	***************************************************************************/
		...
		DWORD dwGrfDex = fdexNameCaseInsensitive)
	{
		...
		if( dwGrfDex & fdexNameCaseSensitive )
		{
			...
			m_mapNameToEntry.Lookup(CString(bstrName),pEntry);
		}
		else
		{
	...
	***************************************************************************/
		...
		bool bTypeInfo = false,bool bUseInDispid = false)
	{
		...
		if( pEntry != NULL )
		{
			...
			m_mapNameToEntry.SetAt(CString(bstrName),pEntry);
			// Copy the string for caseless comparison
			CString strCopy(bstrName);
			// Change the case
			strCopy.MakeLower();
	...
	***************************************************************************/
	...
	HRESULT GetStartDispID(DISPID* pDispID)
	{
		...
		POSITION pos = m_mapDispIDToEntry.GetStartPosition();
		bool bDone = false;
...
// IDispatch
	...
	***************************************************************************/
	...
	STDMETHOD(GetTypeInfoCount)(UINT* pnTypeInfoCount)
	{
		...
		HRESULT hr = AfxIsValidAddress(pnTypeInfoCount,sizeof(UINT)) == TRUE ?
			S_OK : E_INVALIDARG;
		if( SUCCEEDED(hr) )
		{
#ifndef IDISPATCHEX_NO_TYPEINFO
